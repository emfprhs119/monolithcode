...
const RELEASE_VERSION_URL =
  "https://api.github.com/repos/Haeri/MonolithCode2/releases/latest";
const RELEASE_ZIP_URL =
  "https://github.com/Haeri/MonolithCode2/releases/latest/download/";
...
function downloadLatestVersion() {
  axios
    .get()
    .get(RELEASE_ZIP_URL + PLATFORM_ZIP[process.platform], {
      responseType: "stream",
    })
    .then((response) => {
      response.data
        ...
        .on("close", () => {
          ...
          BrowserWindow.getAllWindows().forEach((w) => {
            w.webContents.send("print", {
              text: `new version ${newVersion} will be installed after restart.`,
            });
          });
        });
    });
  ...
  axios
    ...
    .then((response) => {
      ...
      if (appInfo.version !== info.tag_name) {
        ...
        if (
          parseInt(currArr[0], 10) < parseInt(latestArr[0], 10) ||
